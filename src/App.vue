<template>
  <form @submit.prevent="downloadPdf" class="bg-gray-50 min-h-screen py-12 px-6">
    <div class="space-y-12 max-w-3xl mx-auto">
      ...
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div>
          ...
          <input id="naam" name="naam" v-model="form.naam" type="text" required
            class="mt-2 block w-full rounded-md bg-white px-3 py-2 text-gray-900 shadow-sm border border-gray-300 focus:outline-none focus:ring-2 focus:ring-purple-600" />
        </div>
        <div>
      ...
      <div>
        ...
        <div v-for="(actie, index) in form.acties" :key="index"
          ...
          <div class="mb-4">
            ...
            <div class="flex flex-col sm:flex-row gap-4">
              <div
                class="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-purple-300 rounded-lg p-4 bg-purple-50 text-center cursor-pointer hover:bg-purple-100 transition">
          ...
          <div>
            ...
            <textarea :id="`omschrijving-${index}`" :name="`omschrijving-${index}`" v-model="actie.omschrijving" rows="3"
              ...
              class="mt-2 block w-full rounded-md bg-white px-3 py-2 text-gray-900 shadow-sm border border-gray-300 focus:outline-none focus:ring-2 focus:ring-purple-600"></textarea>
          </div>
        ...
        <button @click.prevent="addAction" type="button"
          class="rounded-md bg-purple-600 px-3 py-2 text-sm font-semibold text-black shadow-sm hover:bg-purple-700">Voeg
      ...
      <div class="pt-8 border-t border-gray-900/10 text-right">
        <button @click.prevent="downloadPdf" type="submit"
          class="rounded-md bg-purple-600 px-6 py-2 text-sm font-semibold text-black shadow-sm hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500">
          Download PDF
        </button>
      </div>
    </div>
